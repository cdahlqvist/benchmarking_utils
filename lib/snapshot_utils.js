  ...
  esClient.indices.existsAlias({ name: alias }, function (err, resp) {   
    if(resp) {
      esClient.indices.stats({
    	...
    	var log_msg = 'Alias ' + alias + ' does not currently exist.';
    	log(log_msg);
    	callback();
    }
  });
  ...
  esClient.indices.recovery({ index: index_name, detailed: true }, function(err, resp) {
    if(err) {
      ...
      if(is_completed(resp)) {
        ...
        setTimeout(wait_for_completion, 60000, esClient, index_name, alias, callback);
      }
    }
  });
}
...
function log_aggregated_index_stats(response, alias) {
  ...
  for(var index in stats) {
    docs += (stats[index].primaries.docs.count - stats[index].primaries.docs.deleted);
    primary_size += stats[index].primaries.store.size_in_bytes;
    total_size += stats[index].total.store.size_in_bytes;
  }

  log('Alias ' + alias + ' => documents: ' + docs + ' primary size: ' + primary_size + ' total size: ' + total_size);
}

function log(msg) {
  console.log('%s copysnapshot %s', new Date().toISOString(), msg);
}
